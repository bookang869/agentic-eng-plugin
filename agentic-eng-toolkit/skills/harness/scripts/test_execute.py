"""
Safety-net tests for the execute.py refactor.
Verifies that behavior is identical before and after the refactor.
"""
...
def tmp_project(tmp_path):
    """Temporary project structure with phases/, CLAUDE.md, docs/."""
...
def executor(tmp_project, phase_dir):
    """StepExecutor instance for tests. git calls need to be mocked separately."""
    with patch.object(ex, "ROOT", tmp_project):
        inst = ex.StepExecutor("0-mvp")
    # Reset internal paths to be relative to tmp_project
...
class TestJsonHelpers:
    ...
    def test_save_ensures_ascii_false(self, tmp_path):
        p = tmp_path / "test.json"
        ex.StepExecutor._write_json(p, {"café": "test"})
        raw = p.read_text()
        assert "café" in raw
        assert "\\u" not in raw
...
class TestLoadGuardrails:
    ...
    def test_empty_project(self, tmp_path):
        with patch.object(ex, "ROOT", tmp_path):
            # temporary instance since this is static-like behavior that doesn't need an executor
